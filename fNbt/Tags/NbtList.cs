...
{
    [NotNull] private readonly List<NbtTag> tags = new();
    ...
    public NbtTagType ListType
    {
        ...
        set
        {
            ...
            if (tags.Count > 0)
            {
                var actualType = tags[0].TagType;
                // We can safely assume that ALL tags have the same TagType as the first tag.
                if (actualType != value)
                {
                    var msg = string.Format("Given NbtTagType ({0}) does not match actual element type ({1})",
                        value, actualType);
    ...
    object ICollection.SyncRoot => (tags as ICollection).SyncRoot;
